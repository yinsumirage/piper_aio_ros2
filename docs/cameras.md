...
- 写入正式角色配置后，`camera_f/l/r` 的 status 均通过 type、唯一 publisher、RGB8、640×480 和
  timestamp 门禁。修复父目录创建和 file compression 退出截断后，9.864 秒正式角色
  zstd message-compressed bag 自动读回成功：三路各 297 帧，共 891 帧；按 recorder 接收时间计算
  front/left/right 分别约 30.01/30.07/30.05 Hz，均无超过 100 ms 的接收 gap。bag 中部三路 PNG
  抽帧成功，采样时间彼此最多相差约 10 ms。该短包证明当前绑定与录制 pipeline 可用，不是
